...
class User_script_folders(models.Model):
    ...
    folder_id = models.BigIntegerField(max_length=20)
    ...
    def get_scripts(self, user_id, folder_id):
        with connection.cursor() as cursor:
            cursor.execute("SELECT s.unique_id, s.script_name FROM scripts as s join user_script_folders as usf on usf.user_id = s.user_id WHERE usf.user_id = %s and \
                           usf.user_folder_id = %s and usf.permissions != 0", [user_id, folder_id])
            scripts = []
            row = cursor.fetchone()
            while row:
                scripts.append({"id":row[0], "name":row[1]})
                row = cursor.fetchone()
        return scripts